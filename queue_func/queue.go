...
func NewWorker(workerPool chan chan Job, work http.HandlerFunc) (*Worker, error) {
	if workerPool == nil {
...
func NewDispatcher(maxWorkers int, maxQueueSize int, work http.HandlerFunc) (*Dispatcher, error) {
	if maxWorkers <= 0 {
...
func NewQueue(maxWorkers int, maxQueueSize int, work http.HandlerFunc) *Queue {
	if maxWorkers <= 0 {
		err := fmt.Errorf(`maxWorkers cannot be less than 0`)
		logs.Error(err)
		panic(err)
